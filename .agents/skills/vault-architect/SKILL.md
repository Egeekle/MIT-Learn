...
Capability skill. Called by `/evolve`. The gate procedure itself lives in `00-System/Architecture.md` §7 and `.Codex/rules/architecture-gate.md`; this skill applies it and does not restate it.
...
- After approval: updated Architecture, Change-Log, skills/templates, and `AGENTS.md` only if persistent behavior changed.
- A validation report.

## Vault interaction
Reads everything under `00-System/`, `.Codex/`, and the folder tree. Writes only system files, and only after approval. Does not edit knowledge notes.
...
4. Skills do not restate `AGENTS.md` or rule text.
